...
use guest::randomx::blake2b::blake2b_256;
use guest::randomx::program::Program;
...
pub struct ProgramSegmentInput {
    ...
    pub difficulty: u64,
    /// Pre-computed program instructions (256 instructions × 8 bytes each)
    /// Generated by host from seed using AES
    pub program_instructions: Vec<u8>,
    /// Pre-computed program entropy (16 × 8 bytes)
    #[serde(with = "serde_big_array::BigArray")]
...
fn verify_merkle_proof(
    ...
    // Hash the item to get the leaf (using accelerated SHA-256)
    ...
    // Walk up the tree using accelerated SHA-256
    let mut idx = index;
...
/// Uses Vec instead of BTreeMap for lower overhead with ~64 items
...
impl VerifiedDataset {
    fn new(
        ...
        // Verify each entry's Merkle proof and build the lookup table
        for entry in entries {
            ...
            items.push((entry.index, entry.item));
        }
    ...
    fn get_dataset_item(&self, item_index: u64) -> [u8; 64] {
        // Linear search for ~64 items (faster than BTreeMap overhead)
        for (idx, item) in &self.items {
            if *idx == item_index {
                return *item;
            }
        }
        panic!("Dataset item {} not provided in input", item_index)
    }
}
...
fn main() {
    ...
    // Use pre-computed program from host (saves AES generation cycles)
    let program = Program::from_bytes(&input.program_instructions, input.program_entropy);
    ...
    let (pow_hash, difficulty_valid) = if input.is_last && is_program_complete {
        // Read first 64 bytes of scratchpad directly (no clone needed)
        let mut sp_hash = [0u8; 64];
        sp_hash.copy_from_slice(&vm.scratchpad[0..64]);
